...
class PriorityProcessingQueue(ProcessingQueue):
    """优先级处理队列"""
    ...
    def __init__(self, max_size: int = 100):
        ...
        self.processing_metadata: Dict[str, Dict] = {}
        self.lock = threading.Lock()
        self.logger = logging.getLogger('project_bach.priority_queue')
        self._counter = 0  # 用于保证相同优先级时的FIFO顺序
    
    def add_file(self, file_path: str, priority: int = 5, metadata: Dict = None) -> bool:
        """添加文件到优先级队列
        ...
        with self.lock:
            ...
            try:
                # 优先级队列项目：(priority, counter, file_path)
                self._counter += 1
                self.queue.put((priority, self._counter, file_path), block=False)
                self.processing_status[file_path] = ProcessingStatus.PENDING
                self.processing_metadata[file_path] = {
                    'added_time': datetime.now(),
                    'priority': priority,
                ...
                self.logger.info(f"文件已添加到优先级队列 (优先级 {priority}): {file_path}")
                return True
    ...
    def get_file(self, timeout: float = 1.0) -> Optional[str]:
        ...
        """
        try:
            priority, counter, file_path = self.queue.get(timeout=timeout)
            with self.lock:
                self.processing_status[file_path] = ProcessingStatus.PROCESSING
                if file_path in self.processing_metadata:
                    self.processing_metadata[file_path]['start_time'] = datetime.now()
            
            self.logger.debug(f"从优先级队列获取文件 (优先级 {priority}): {file_path}")
            return file_path
