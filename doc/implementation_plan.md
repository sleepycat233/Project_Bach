...
- 🔴 **Phase 11**: Speaker Diarization时间戳对齐算法优化 - **当前开发中**
...
**目标**: 解决HuggingFace ASRDiarizationPipeline算法中重复内容分配问题

**核心问题**:
- 同一段音频内容被分配给多个说话人
- 时间重叠的speaker segments导致重复文本段落
- 当前算法在处理重叠segments时存在chunk索引更新逻辑失效

**技术方案**:
- 📋 重叠检测与处理：检测speaker segments重叠，优先分配给覆盖范围更大的segment
- 📋 Chunk分割策略：跨越多个speaker的chunk进行智能分割
- 📋 时间窗口投票：基于chunk整个时间窗口的speaker覆盖比例进行分配
- 📋 后处理去重：group_by_speaker后检测并合并时间重叠的段落

### 📋 Phase 7: 前端Post-Processing选择器 (中等优先级)

**目标**: 实现用户可选择性启用/禁用后处理步骤的灵活化流程

**核心功能**:
- 📋 **NER匿名化选择器**: 用户可选择跳过人名匿名化处理
- 📋 **AI摘要生成开关**: 可选择性启用/禁用摘要生成节省成本
- 📋 **思维导图生成控制**: 根据内容类型智能默认或用户手动选择
- 📋 **配置持久化**: 基于content type的智能默认值系统

**技术实现**:
- 前端UI增加post-processing选项面板
- 后端AudioProcessor根据用户选择条件化执行各步骤
- 配置系统支持基于content type的智能默认值
- API性能优化：跳过不需要的AI调用节省时间和费用
