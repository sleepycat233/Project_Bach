...
class TestTranscriptStorage(unittest.TestCase):
    ...
    def test_save_raw_transcript(self):
        ...
        saved_path = self.storage.save_raw_transcript(filename, content)
    ...
    def test_save_anonymized_transcript(self):
        ...
        saved_path = self.storage.save_anonymized_transcript(filename, content)
    ...
    def test_list_transcripts(self):
        """测试列出转录文件"""
        # 保存一些测试文件
        self.storage.save_raw_transcript("test1", "content1")
        self.storage.save_anonymized_transcript("test2", "content2")
        
        transcripts = self.storage.list_transcripts()
        
        self.assertGreaterEqual(len(transcripts), 2)
        transcript_names = [os.path.basename(t) for t in transcripts]
        self.assertTrue(any('test1' in name and 'raw' in name for name in transcript_names))
        self.assertTrue(any('test2' in name and 'anonymized' in name for name in transcript_names))
    ...
    def test_load_transcript(self):
        ...
        # 保存文件
        saved_path = self.storage.save_raw_transcript(filename, content)
        transcript_filename = os.path.basename(saved_path)
        
        # 加载文件
        loaded_content = self.storage.load_transcript(transcript_filename)
        
        self.assertEqual(loaded_content, content)
    
    def test_load_nonexistent_transcript(self):
        """测试加载不存在的转录文件"""
        result = self.storage.load_transcript("nonexistent.txt")
    ...
    def test_delete_transcript(self):
        ...
        # 删除文件
        result = self.storage.delete_transcript(transcript_filename)
    ...
    def test_get_transcript_info(self):
        ...
        # 获取信息
        info = self.storage.get_transcript_info(transcript_filename)
...
class TestResultStorage(unittest.TestCase):
    ...
    def test_save_markdown_result(self):
        ...
        saved_path = self.storage.save_markdown_result(filename, results)
    ...
    def test_save_json_result(self):
        ...
        saved_path = self.storage.save_json_result(filename, results)
    ...
    def test_save_html_result(self):
        ...
        saved_path = self.storage.save_html_result(filename, results)
    ...
    def test_list_results(self):
        ...
        self.storage.save_markdown_result("test1", results1)
        self.storage.save_json_result("test2", results2)
        
        result_files = self.storage.list_results()
        
        self.assertGreaterEqual(len(result_files), 2)
        file_names = [os.path.basename(f) for f in result_files]
        self.assertTrue(any('test1' in name and name.endswith('.md') for name in file_names))
        self.assertTrue(any('test2' in name and name.endswith('.json') for name in file_names))
    ...
    def test_load_result(self):
        ...
        # 保存文件
        saved_path = self.storage.save_json_result(filename, results)
        result_filename = os.path.basename(saved_path)
        
        # 加载文件
        loaded_data = self.storage.load_result(result_filename)
    ...
    def test_delete_result(self):
        ...
        # 删除文件
        result = self.storage.delete_result(result_filename)
